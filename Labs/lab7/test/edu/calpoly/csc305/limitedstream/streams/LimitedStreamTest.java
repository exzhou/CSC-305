...
class LimitedStreamTest {
  ...
  void countEmptyTest() {
    assertEquals(0,
        LimitedStream.from(List.of().iterator())
            .count());
  }
  ...
  void countTest() {
    assertEquals(3,
        LimitedStream.from(List.of("a", "b", "c").iterator())
            .count());
  }
  ...
  void limitFewerThanStreamTest() {
    assertEquals(4,
        LimitedStream.from(new FromIterator(0))
            .limit(4)
            .count());
  }
  ...
  void limitGreaterThanStreamTest() {
    assertEquals(2,
        LimitedStream.from(List.of(1, 2).iterator())
            .limit(4)
            .count());
  }
  ...
  @Test
  void mapAsListTest() {
    assertEquals(List.of(2, 5, 4),
        LimitedStream.from(List.of("hi", "hello", "ciao").iterator())
            .map(String::length)
            .asList());
  }
  ...
  void limitFilterAsListTest() {
    assertEquals(List.of(10, 20, 30, 40, 50),
        LimitedStream.from(new FromIterator(1))
            .filter(n -> n % 10 == 0)
            .limit(5)
            .asList());
  }
